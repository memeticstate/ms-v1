...
export function WatchlistPanel({ entries, launches, onInspect, onRulesChange, onRemove, onReadAll, storageMode = "device", syncMessage = null, transitions = {}, transitionError = null }: {
  ...
  if (!entries.length) {
    return (
      <section className="watchlist-empty grid min-h-[280px] place-items-center overflow-hidden rounded-2xl border border-foreground/10 bg-[var(--surface-1)]/86 px-5 py-10 text-center">
        <div className="max-w-xl">
          ...
          <p className="mt-6 font-medium text-xs tracking-normal text-attention">Research watchlist</p>
          <h2 className="specimen-serif mt-3 text-4xl tracking-[-0.035em] text-foreground/88">Keep an eye on what matters.</h2>
          <p className="mx-auto mt-4 max-w-lg text-sm leading-7 text-muted-foreground">Save a token from its brief. Return here to follow recorded changes and choose what to watch.</p>
  ...
  return (
    <div className="space-y-3">
      <section className="rounded-2xl border border-foreground/10 bg-[var(--surface-1)]/86 p-4 sm:p-5">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <div className="flex items-center gap-2"><Bookmark className="size-3.5 text-attention" /><p className="font-medium text-caption tracking-normal text-attention">Research watchlist</p></div>
            <h2 className="specimen-serif mt-2 text-3xl tracking-[-0.035em] text-foreground/88">Your watchlist.</h2>
            <p className="mt-2 text-xs leading-5 text-muted-foreground">Rules run against each verified refresh while the observatory is open. Signed-in Passport watches also persist across devices; background delivery remains a later, usage-gated service.</p>
          </div>
          <div className="flex items-center gap-2">
            <span className="rounded border border-foreground/10 bg-foreground/[0.025] px-3 py-2 font-medium text-xs tracking-normal text-muted-foreground">{storageMode === "passport" ? "Passport sync" : "This device"}</span>
            <span className="rounded border border-foreground/10 bg-foreground/[0.025] px-3 py-2 font-medium text-xs tracking-normal text-muted-foreground">{entries.length} saved · {rules} rules</span>
            {unread ? <Button type="button" variant="outline" size="sm" onClick={onReadAll} className="border-signal/20 bg-signal/[0.035] font-medium text-xs tracking-normal text-signal"><BellRing />Mark {unread} read</Button> : null}
          </div>
        </div>
      </section>

      {syncMessage ? <div role="status" className="rounded border border-culture/14 bg-culture/[0.025] px-3 py-2 font-medium text-xs tracking-normal text-culture/75">{syncMessage}</div> : null}
      ...
      <div className="grid gap-3 xl:grid-cols-2">
        {entries.map((entry) => {
          ...
          return (
            <article key={entry.tokenAddress} className="watch-card relative overflow-hidden rounded-2xl border border-foreground/10 bg-[var(--surface-2)]/88">
              ...
              <div className="grid grid-cols-2 gap-px bg-foreground/[0.07] sm:grid-cols-4">
                {[
                  ...
                  ["Trade change", (launch ? launch.momentumPercent : entry.lastSeen.momentumPercent) == null ? "—" : `${launch ? launch.momentumPercent : entry.lastSeen.momentumPercent}%`],
                  ["Lifecycle", launch?.phase ?? entry.lastSeen.phase],
                ].map(([label, value]) => <div key={label} className="bg-[var(--surface-2)] px-3 py-3"><p className="font-medium text-xs tracking-normal text-muted-foreground">{label}</p><p className="mt-1 truncate font-mono text-caption uppercase text-muted-foreground">{value}</p></div>)}
              </div>

              <details className="watch-rules"><summary>Watch rules <span>Adjust what to follow</span></summary><div className="grid gap-2 p-4 sm:grid-cols-2 sm:p-5">
                ...
                <label className="rounded border border-foreground/8 bg-foreground/[0.018] px-3 py-2.5"><span className="mb-2 flex items-center gap-1.5 font-medium text-xs tracking-normal text-muted-foreground"><Activity className="size-3 text-attention" />Activity crosses</span><Select value={entry.rules.activityThreshold === null ? "off" : String(entry.rules.activityThreshold)} onValueChange={(value) => onRulesChange(entry.tokenAddress, { ...entry.rules, activityThreshold: updateThreshold(value) })}><SelectTrigger aria-label={`Activity rule for ${entry.symbol}`} size="sm" className="w-full border-foreground/10 bg-foreground/[0.025] font-mono text-xs text-muted-foreground"><SelectValue /></SelectTrigger><SelectContent className="border-foreground/10 bg-[var(--surface-popover)] text-foreground/70"><SelectItem value="off">Rule off</SelectItem><SelectItem value="10">10 recent trades</SelectItem><SelectItem value="25">25 recent trades</SelectItem><SelectItem value="50">50 recent trades</SelectItem><SelectItem value="100">100 recent trades</SelectItem></SelectContent></Select></label>
                <label className="rounded border border-foreground/8 bg-foreground/[0.018] px-3 py-2.5"><span className="mb-2 flex items-center gap-1.5 font-medium text-xs tracking-normal text-muted-foreground"><TrendingUp className="size-3 text-signal" />Momentum crosses</span><Select value={entry.rules.momentumThreshold === null ? "off" : String(entry.rules.momentumThreshold)} onValueChange={(value) => onRulesChange(entry.tokenAddress, { ...entry.rules, momentumThreshold: updateThreshold(value) })}><SelectTrigger aria-label={`Momentum rule for ${entry.symbol}`} size="sm" className="w-full border-foreground/10 bg-foreground/[0.025] font-mono text-xs text-muted-foreground"><SelectValue /></SelectTrigger><SelectContent className="border-foreground/10 bg-[var(--surface-popover)] text-foreground/70"><SelectItem value="off">Rule off</SelectItem><SelectItem value="25">+25%</SelectItem><SelectItem value="50">+50%</SelectItem><SelectItem value="100">+100%</SelectItem><SelectItem value="200">+200%</SelectItem></SelectContent></Select></label>
              </div></details>

              <div className="border-t border-foreground/8 px-4 py-3 sm:px-5">
                {entry.alerts.length ? <div className="space-y-2">{entry.alerts.slice(0, 3).map((item) => <div key={item.id} className={`flex gap-3 rounded border px-3 py-2.5 ${item.read ? "border-foreground/7 bg-foreground/[0.012]" : "border-signal/14 bg-signal/[0.025]"}`}><span className={`mt-1 size-1.5 shrink-0 rounded-full ${item.read ? "bg-foreground/16" : "bg-signal"}`} /><div className="min-w-0"><p className="text-caption font-medium text-muted-foreground">{item.title}</p><p className="mt-1 truncate font-medium text-xs tracking-normal text-muted-foreground">{item.detail} · {relativeTime(item.observedAt)}</p></div></div>)}</div> : <div className="flex items-center gap-2 font-medium text-xs tracking-normal text-muted-foreground"><RadioTower className="size-3" />No rule crossings observed since this launch was saved</div>}
                {!launch ? <div className="mt-3 flex gap-2 rounded border border-culture/12 bg-culture/[0.025] p-3 text-caption leading-5 text-muted-foreground"><GitBranch className="mt-0.5 size-3.5 shrink-0 text-culture" />This launch is outside the current ranked result window. Its saved rules and last verified state remain on this device.</div> : null}
              </div>
            </article>
          );
        })}
