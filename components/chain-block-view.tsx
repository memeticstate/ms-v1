    ...
    <div className="space-y-3">
        ...
        <div className="relative grid min-h-[660px] lg:grid-cols-[0.58fr_1.42fr]">
          <div className="flex flex-col justify-between border-b border-foreground/10 p-6 lg:border-b-0 lg:border-r lg:p-9">
            <div>
              <div className="flex items-center justify-between gap-3">
                <p className="font-mono text-caption uppercase tracking-[0.24em] text-danger">Chain Block</p>
                <span className="rounded-full border border-foreground/10 px-2.5 py-1 font-mono text-xs uppercase tracking-[0.13em] text-muted-foreground">
              ...
              <p className="mt-2 font-mono text-caption uppercase tracking-[0.18em] text-muted-foreground">{snapshot.block.date}</p>
              ...
              <div className="mt-8 grid grid-cols-2 gap-px overflow-hidden rounded-md border border-foreground/10 bg-foreground/10">
                <div className="bg-[var(--surface-popover)] p-3">
                  <p className="font-mono text-xs uppercase tracking-[0.14em] text-muted-foreground">Confidence</p>
                ...
                <div className="bg-[var(--surface-popover)] p-3">
                  <p className="font-mono text-xs uppercase tracking-[0.14em] text-muted-foreground">Materiality</p>
            ...
            <div className="mt-10 space-y-5">
              <div>
                <p className="font-mono text-xs uppercase tracking-[0.18em] text-muted-foreground">Constituent species</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {featured.length ? featured.map((species) => (
                    <span key={species.id} className="rounded-full border bg-black/10 px-3 py-1.5 font-mono text-caption"
                      style={{ borderColor: `${species.color}66`, color: species.color }}>
                      {species.symbol}
                    </span>
                  )) : <span className="font-mono text-caption uppercase tracking-[0.12em] text-muted-foreground">Historical cohort</span>}
                </div>
              </div>
              <div className="space-y-2 border-t border-foreground/10 pt-4 font-mono text-xs uppercase tracking-[0.12em] text-muted-foreground">
          ...
          <div className="flex flex-col p-6 lg:p-10 xl:p-12">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <p className="font-mono text-xs uppercase tracking-[0.2em] text-signal">Primary material state change</p>
              <span className="font-mono text-xs uppercase tracking-[0.14em] text-muted-foreground">{engineLabel}</span>
            </div>
            ...
            <p className="mt-5 max-w-3xl text-sm leading-7 text-muted-foreground">{snapshot.block.interpretation}</p>
            ...
            <div className="mt-8 grid grid-cols-2 gap-px overflow-hidden rounded-md border border-foreground/10 bg-foreground/10 lg:grid-cols-4">
              {snapshot.block.evidence.map((item) => (
                <div key={item.label} className="bg-[var(--surface-popover)] p-4">
                  <p className="font-mono text-xs uppercase tracking-[0.14em] text-muted-foreground">{item.label}</p>
            ...
            <div className="mt-8">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-mono text-xs uppercase tracking-[0.17em] text-muted-foreground">Material drivers</p>
                </div>
                <span className="font-mono text-xs uppercase tracking-[0.12em] text-muted-foreground">All threshold crossings retained</span>
              </div>
                ...
                <div className="grid gap-2 md:grid-cols-3">
                  {snapshot.block.drivers.map((driver) => (
                    <div key={`${driver.kind}-${driver.label}`} className="rounded-md border border-foreground/10 bg-black/10 p-3">
                      <div className="flex items-center justify-between gap-3">
                        <p className="font-mono text-xs uppercase tracking-[0.13em]" style={{ color: kindTone[driver.kind] ?? "var(--slate)" }}>
                          {driver.label}
                        </p>
                        <span className="font-mono text-xs text-muted-foreground">{driver.significance}/100</span>
                      </div>
                      <p className="mt-2 text-caption leading-5 text-muted-foreground">{driver.summary}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="rounded-md border border-dashed border-foreground/10 px-4 py-5 text-xs leading-5 text-muted-foreground">
                  This is the authored baseline. The next material verified difference will begin the multi-driver ledger.
                </div>
              )}
            </div>

            <div className="mt-auto flex items-center justify-between border-t border-foreground/10 pt-4 font-mono text-xs uppercase tracking-[0.13em] text-muted-foreground">
      ...
      <section className="overflow-hidden rounded-[10px] border border-foreground/10 bg-[var(--surface-2)]">
        <div className="flex items-center justify-between gap-3 border-b border-foreground/10 px-4 py-3">
          <div>
            <p className="font-mono text-xs uppercase tracking-[0.18em] text-attention">State memory</p>
            <p className="mt-1 text-xs text-muted-foreground">Material blocks retained independently from routine snapshots</p>
          </div>
          <span className="font-mono text-xs uppercase tracking-[0.13em] text-muted-foreground">{history.length} derived blocks</span>
        </div>
        {history.length ? (
          <div className="grid divide-y divide-white/8 lg:grid-cols-4 lg:divide-x lg:divide-y-0">
            {history.slice(0, 4).map((record) => (
              <article key={record.chainBlockId} className="p-4">
                <div className="flex items-center justify-between gap-3">
                  <span className="font-mono text-xs uppercase tracking-[0.14em]" style={{ color: kindTone[record.kind] ?? "var(--slate)" }}>
                    Block {record.number} · {record.kind}
                  </span>
                  <span className="font-mono text-xs text-muted-foreground">{record.significance}</span>
                </div>
                <h3 className="mt-3 text-sm font-semibold tracking-tight text-foreground/75">{record.block.title}</h3>
                <p className="mt-2 line-clamp-2 text-caption leading-4 text-muted-foreground">{record.block.signal}</p>
                <p className="mt-3 font-mono text-xs uppercase tracking-[0.11em] text-muted-foreground">
          ...
          <div className="px-4 py-6 text-xs text-muted-foreground">The first v2 material difference will appear here automatically.</div>
        )}
      </section>
    </div>
  );
