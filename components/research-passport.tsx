  ...
  if (!passport) {
    return (
      <section id="passport" className="passport-ledger grid min-h-64 place-items-center rounded-[9px] border border-foreground/10 bg-[var(--surface-1)]/90 p-8 text-center">
        <div><RefreshCw className="mx-auto size-5 animate-spin text-culture/55" /><p className="mt-4 font-mono text-xs uppercase tracking-[0.16em] text-muted-foreground">Opening research passport</p></div>
      </section>
    );
  }
    ...
    return (
      <section id="passport" className="passport-ledger relative overflow-hidden rounded-[9px] border border-culture/18 bg-[var(--surface-1)]/92 p-5 sm:p-7">
        <div className="grid gap-7 lg:grid-cols-[1fr_.85fr] lg:items-end">
          <div>
            <div className="flex items-center gap-2"><Fingerprint className="size-4 text-culture" /><p className="font-mono text-xs uppercase tracking-[0.18em] text-culture">Research Passport · entitlement v1</p></div>
          ...
          <div className="rounded border border-foreground/10 bg-[var(--surface-depth)]/70 p-4">
            {usesPrivy ? (
              <button type="button" disabled={!privyReady || loginState === "pending"} onClick={signIn} className="flex w-full items-center justify-center gap-2 rounded border border-culture/30 bg-culture/[0.075] px-4 py-3 font-mono text-xs uppercase tracking-[0.14em] text-culture transition hover:bg-culture/[0.12] disabled:cursor-wait disabled:opacity-55"><WalletCards className="size-3.5" />{loginState === "pending" ? "Opening secure login" : "Open with wallet or email"}</button>
            ) : passport.signInPath ? (
              <a href={passport.signInPath} target="_top" className="flex items-center justify-center gap-2 rounded border border-culture/30 bg-culture/[0.075] px-4 py-3 font-mono text-xs uppercase tracking-[0.14em] text-culture transition hover:bg-culture/[0.12]"><KeyRound className="size-3.5" />Sign in to open Passport</a>
            ) : null}
            {usesPrivy && loginState === "error" ? <p className="mt-3 text-center font-mono text-xs uppercase tracking-[0.09em] text-danger/75">Login was cancelled or unavailable</p> : null}
            {passport.error ? <p className="mt-3 text-center font-mono text-xs uppercase tracking-[0.09em] text-danger/75">Secure session unavailable · {passport.error.replaceAll("_", " ")}</p> : null}
            <p className="mt-3 text-center font-mono text-xs uppercase tracking-[0.09em] text-muted-foreground">Public signals, rankings and evidence remain open</p>
          </div>
        </div>
      </section>
    );
        ...
        <div className="rounded border border-foreground/10 bg-[var(--surface-depth)]/72 p-4 sm:p-5">
          ...
          <div className="mt-5 grid gap-px overflow-hidden rounded border border-foreground/10 bg-foreground/10 sm:grid-cols-2">
            {capacities.map(({ label, key, icon: Icon }) => {
              ...
              return <div key={key} className="bg-[var(--surface-2)] p-4"><div className="flex items-center justify-between"><p className="font-mono text-xs uppercase tracking-[0.11em] text-muted-foreground">{label}</p><Icon className="size-3.5 text-culture/60" /></div><p className="mt-3 font-mono text-xl text-foreground/78">{number.format(entitlements.remaining[key])}<span className="ml-1 text-xs uppercase tracking-[0.08em] text-muted-foreground">left</span></p><p className="mt-1 font-mono text-xs uppercase tracking-[0.08em] text-muted-foreground">{number.format(used)} used · {number.format(allowance)} allowance</p></div>;
            })}
          </div>
          <div className={`mt-3 flex gap-3 rounded border p-3 ${passport.premiumAccess.active ? "border-signal/24 bg-signal/[0.045]" : "border-culture/15 bg-culture/[0.03]"}`}><ShieldCheck className={`mt-0.5 size-4 shrink-0 ${passport.premiumAccess.active ? "text-signal" : "text-culture"}`} /><div><p className={`font-mono text-xs uppercase tracking-[0.12em] ${passport.premiumAccess.active ? "text-signal" : "text-culture"}`}>{passport.premiumAccess.active ? "Premium Interpretation · active" : `Premium Interpretation · ${passport.premiumAccess.status.replace("_", " ")}`}</p><p className="mt-1 text-caption leading-5 text-muted-foreground">{passport.premiumAccess.reason}</p>{passport.premiumAccess.active ? <p className="mt-2 font-mono text-xs uppercase tracking-[0.09em] text-signal/70">0.1% threshold · block {passport.premiumAccess.blockNumber?.toLocaleString() ?? "—"}</p> : null}</div></div>
          <div className="mt-3 flex flex-wrap gap-1.5">{passport.publicGuarantees.map((item) => <span key={item} className="rounded border border-signal/13 bg-signal/[0.025] px-2 py-1 font-mono text-xs uppercase tracking-[0.09em] text-signal/70">{item} · public</span>)}</div>
        </div>
      </div>
    </section>
  );
