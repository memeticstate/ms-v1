  ...
  if (!engine) {
    return (
      <section className="mt-3 rounded-[10px] border border-foreground/10 bg-[var(--surface-2)]/72 p-4">
        <p className="font-mono text-caption uppercase tracking-[0.2em] text-culture">Engine telemetry initializing</p>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="mt-2 text-xs leading-5 text-muted-foreground">The collector has not committed its first observable run yet. Opening the snapshot endpoint also activates the watchdog.</p>
          <button type="button" onClick={wake} disabled={wakeState === "sending"}
            className="rounded border border-signal/25 bg-signal/8 px-3 py-2 font-mono text-xs uppercase tracking-[0.14em] text-signal disabled:opacity-40">
      ...
      <div className="grid gap-px bg-foreground/[0.07] sm:grid-cols-2 xl:grid-cols-5">
        <div className="bg-[var(--surface-3)] p-4 sm:col-span-2 xl:col-span-1">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            ...
            <span className="font-mono text-caption uppercase tracking-[0.15em] text-muted-foreground">/ 100 · {engine.status}</span>
          </div>
        </div>
        <div className="bg-[var(--surface-3)] p-4">
          ...
          <p className={`mt-2 font-mono text-caption uppercase tracking-[0.1em] ${tone(run?.status ?? "pending")}`}>{run?.status ?? "pending"} · {run?.phase ?? "awaiting genesis"}</p>
          <p className="mt-1 font-mono text-xs uppercase tracking-[0.12em] text-muted-foreground">{run ? `${run.trigger} · ${duration(run.durationMs)}` : "watchdog armed"}</p>
          <button type="button" onClick={wake} disabled={wakeState === "sending"}
            className="mt-2 font-mono text-xs uppercase tracking-[0.12em] text-signal/65 hover:text-signal disabled:opacity-30">
        ...
        <div className="bg-[var(--surface-3)] p-4">
          ...
          <p className="mt-2 font-mono text-caption uppercase tracking-[0.1em] text-foreground/70">{engine.archive.snapshots} snapshots · {engine.archive.speciesObservations} observations</p>
          <p className="mt-1 font-mono text-xs uppercase tracking-[0.12em] text-muted-foreground">{engine.archive.chainBlocks} material chain blocks</p>
        </div>
        <div className="bg-[var(--surface-3)] p-4">
          ...
          <p className="mt-2 font-mono text-caption uppercase tracking-[0.1em] text-foreground/70">{engine.registry.assets} assets · {engine.registry.quotedHabitats} quoted</p>
          <p className="mt-1 font-mono text-xs uppercase tracking-[0.12em] text-muted-foreground">registry + prices + RPC evidence</p>
        </div>
        <div className="bg-[var(--surface-3)] p-4">
          ...
          <p className={`mt-2 font-mono text-caption uppercase tracking-[0.1em] ${engine.alerts.length ? "text-danger" : "text-signal"}`}>{engine.alerts.length ? `${engine.alerts.length} require attention` : "No unresolved anomalies"}</p>
          <p className="mt-1 truncate font-mono text-xs uppercase tracking-[0.12em] text-muted-foreground">{engine.alerts[0]?.code ?? "evidence lanes nominal"}</p>
        </div>
      </div>
            ...
            {engine.sources.map((item) => (
              <div key={item.source} className="min-w-[170px] flex-1 rounded border border-foreground/[0.07] bg-foreground/[0.025] px-3 py-2">
                <div className="flex items-center justify-between gap-2">
                  <span className="truncate font-mono text-xs uppercase tracking-[0.12em] text-muted-foreground">{item.source}</span>
                  <span className={`size-1.5 shrink-0 rounded-full bg-current ${tone(item.status)}`} />
                </div>
                <p className="mt-1.5 font-mono text-xs uppercase tracking-[0.1em] text-muted-foreground">{duration(item.latencyMs)} · {item.recordCount} records · age {age(item.freshnessMs)}</p>
              </div>
            ))}
          </div>
        </div>
