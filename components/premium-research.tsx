...
import { EXPLORER, type ResearchDossier } from "@/lib/premium/research";
...
function ResearchContent({ data, loadOlder, loadNewest, loading, olderPage }: { data: ResearchDossier; loadOlder: () => void; loadNewest: () => void; loading: boolean; olderPage: boolean }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <section className={box}><p className={label}>Identity and source records</p><dl className="mt-4 space-y-3 text-sm">{[["Token", data.token.address], ["Curve", data.token.curveAddress], ["Creator", data.token.creator], ["Quote asset", data.token.quoteAddress]].map(([name, address]) => <div key={name} className="flex flex-wrap items-center justify-between gap-2"><dt className="text-muted-foreground">{name}</dt><dd><a href={`${EXPLORER}/address/${address}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 break-all font-mono underline underline-offset-4" title={address}>{short(address)}<ExternalLink className="size-3" /></a></dd></div>)}</dl><p className="mt-5 text-sm leading-6 text-muted-foreground">Launch: {time(data.token.launchedAt)} · <a className="underline" href={`${EXPLORER}/tx/${data.token.launchTransaction}`} target="_blank" rel="noreferrer">View launch transaction</a></p></section>
...
export function PremiumResearchWorkspace({ passport, launches, initialToken, onRefreshPassport }: { passport: PassportResponse | null; launches: PonsLaunchView[]; initialToken: string | null; onRefreshPassport: (sync?: boolean) => Promise<void> }) {
  ...
  const [workspaceTab, setWorkspaceTab] = useState("researcher");
  ...
  return <section className="space-y-4" aria-label="Premium Field Operator">
    <details className={`${box} text-sm leading-6`}><summary className="cursor-pointer text-base font-semibold">How to use premium research</summary><ol className="mt-4 list-decimal space-y-2 pl-5"><li>Choose a token and open a 25,000-block window. Check its coverage date before interpreting the counts.</li><li>Read the history and source relationships. Write one thesis and a condition that would prove it wrong.</li><li>Save the window as a private case. Return to Case files, compare the latest indexed evidence, and record your outcome.</li></ol><p className="mt-3 text-muted-foreground">For discovery while the archive is delayed, use the fresh factory feed and the separate Recent curve check. A comparison with no newer indexed blocks does not show a new market outcome.</p><a href="/docs/premium" className="mt-3 inline-block underline underline-offset-4">Read the worked example</a></details>
    <div className={`${box} border-attention/25`}><div className="flex flex-wrap items-start justify-between gap-4"><div><p className={label}>Premium Field Operator</p><h2 className="specimen-serif mt-2 text-3xl sm:text-4xl">Follow the evidence further.</h2><p className="mt-3 max-w-3xl text-base leading-7 text-muted-foreground">Saved research assignments, dated evidence, and private case files for tokens on Robinhood Chain.</p></div><span className="inline-flex items-center gap-2 rounded border border-attention/25 px-3 py-2 text-sm text-attention">{access?.active ? <CheckCircle2 className="size-4" /> : <LockKeyhole className="size-4" />}0.1% holder access</span></div>
      {!access?.active ? <div className="mt-6"><div className="grid gap-3 md:grid-cols-3">{[[Search, "Expanded evidence", "Selectable windows, dated sources, and traceable relationships."], [History, "Indexed history", "Older records and three comparable observation windows."], [BookOpen, "Research notes", "Research prompts, invalidation paths, and saved private cases."]].map(([Icon, title, copy]) => { const Mark = Icon as typeof Search; return <div key={String(title)} className="rounded border border-foreground/10 bg-[var(--surface-2)] p-4"><Mark className="size-5 text-attention" /><h3 className="mt-3 text-base font-semibold">{String(title)}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{String(copy)}</p></div>; })}</div><p className="mt-5 text-base leading-7 text-muted-foreground">{access?.reason ?? "Connect and verify a wallet to check eligibility. Hold at least 0.1% of the fixed supply reference in the verified primary wallet."}</p><div className="mt-4 flex flex-wrap gap-3">{!authenticated ? <Button onClick={signIn} disabled={!ready}><LockKeyhole />Connect wallet</Button> : <><Button variant="outline" onClick={linkWallet} disabled={!ready}>Link holder wallet</Button><Button onClick={() => void recheck()} disabled={loading}><RefreshCw className={loading ? "animate-spin" : ""} />Recheck access</Button></>}<a href="/docs/premium" className="inline-flex items-center gap-2 px-2 text-sm underline underline-offset-4">Read premium terms<ExternalLink className="size-3" /></a></div></div>
      : <div className="mt-6"><p className="flex flex-wrap items-center gap-2 text-sm text-signal"><ShieldCheck className="size-4" />Verified wallet {short(access.walletAddress ?? "")} · minimum {quantity(access.requiredBalanceRaw, access.tokenDecimals)} tokens</p><p className="mt-2 text-sm text-muted-foreground">Supply reference: block {access.supplyReferenceBlock?.toLocaleString() ?? "pending"}. Access is rechecked at most every two minutes.</p>{workspaceTab === "research" ? <><div className="mt-5 grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
        ...
        <Button onClick={() => void load()} disabled={loading || !/^0x[0-9a-f]{40}$/i.test(token.trim())}><Search />{loading ? "Reading evidence…" : "Open research"}</Button>
      </div>
      ...
      </div><p className="mt-3 text-sm leading-6 text-muted-foreground">Choose a window and select Open research. Older history pages keep the same evidence anchor and filters.</p></> : null}
    ...
    {authenticated && access?.active && workspaceTab === "research" && /^0x[0-9a-f]{40}$/i.test(token.trim()) ? <PremiumRecentCurve key={`${identityVersion}:${token.trim().toLowerCase()}`} token={token.trim().toLowerCase()} /> : null}
    {authenticated && access?.active ? <Tabs value={workspaceTab} onValueChange={setWorkspaceTab}><TabsList aria-label="Premium research workspace"><TabsTrigger value="researcher">Researcher</TabsTrigger><TabsTrigger value="research">Dossier</TabsTrigger><TabsTrigger value="cases">Case files</TabsTrigger></TabsList><TabsContent value="researcher" className="mt-3"><HolderResearcher key={identityVersion} accessExpiresAt={access.expiresAt} initialToken={initialToken} /></TabsContent><TabsContent value="research" className="mt-3 space-y-4">{payload?.dossier ? <><div className="flex justify-end"><Button variant="outline" onClick={() => setWorkspaceTab("cases")}><BookOpen />Save this window as a case</Button></div><ResearchContent data={payload.dossier} loading={loading} olderPage={olderPage} loadOlder={() => void load(payload.dossier?.history.nextCursor ?? undefined)} loadNewest={() => void load()} /></> : <div className={box}><p className="text-base leading-7 text-muted-foreground">Open an indexed token to inspect its evidence, source relationships, and research paths.</p></div>}</TabsContent><TabsContent value="cases" forceMount className="mt-3 data-[state=inactive]:hidden"><PremiumCaseFiles key={identityVersion} dossier={payload?.dossier ?? null} accessExpiresAt={access.expiresAt} /></TabsContent></Tabs> : null}
  </section>;
}
