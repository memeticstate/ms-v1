...
import { CopyContract, TokenLabel } from "@/components/token-identity";
...
const box = "research-panel rounded-2xl border border-foreground/10 bg-[var(--surface-1)] p-5 sm:p-7";
  ...
  return <div className="space-y-4">
    <section className={`${box} border-attention/25`}>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className={label}>Research dossier · PONS V2</p><h3 className="specimen-serif mt-2 text-3xl sm:text-4xl"><TokenLabel token={{tokenAddress: data.token.address, name: data.token.name, symbol: data.token.symbol}} /><span className="ml-3 text-xl text-muted-foreground">{data.token.quoteSymbol} habitat</span></h3></div>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <section className={box}><p className={label}>Identity and source records</p><dl className="mt-4 space-y-3 text-sm">{[["Token", data.token.address], ["Curve", data.token.curveAddress], ["Creator", data.token.creator], ["Quote asset", data.token.quoteAddress]].map(([name, address]) => <div key={name} className="flex flex-wrap items-center justify-between gap-2"><dt className="text-muted-foreground">{name}</dt><dd className="flex items-center gap-2">{name !== "Creator" ? <CopyContract address={address} /> : null}<a href={robinhoodExplorer.address(address)} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 break-all font-mono underline underline-offset-4" >{name === "Creator" ? short(address) : "View contract"}<ExternalLink className="size-3" /></a></dd></div>)}</dl><p className="mt-5 text-sm leading-6 text-muted-foreground">Launch: {time(data.token.launchedAt)} · <a className="underline" href={robinhoodExplorer.tx(data.token.launchTransaction)} target="_blank" rel="noreferrer">View launch transaction</a></p></section>
  ...
  return <section className="research-workspace space-y-5" aria-label="Holder research workspace">
    <div className={`${box} research-intro`}><div className="flex flex-wrap items-start justify-between gap-4"><div><p className={label}>Research for holders</p><h2 className="specimen-serif mt-2 text-3xl sm:text-4xl">{initialTab === "saved" ? "Keep a record. Return with a question." : "What do you want to understand?"}</h2><p className="mt-3 max-w-3xl text-base leading-7 text-muted-foreground">Ask about a token, inspect a dated brief, or return to your saved research.</p></div><span className="inline-flex items-center gap-2 rounded border border-attention/25 px-3 py-2 text-sm text-attention">{access?.active ? <CheckCircle2 className="size-4" /> : <LockKeyhole className="size-4" />}0.1% holder access</span></div>
      ...
      : <div className="mt-6"><p className="flex flex-wrap items-center gap-2 text-sm text-signal"><ShieldCheck className="size-4" />Verified wallet {short(access.walletAddress ?? "")} · minimum {quantity(access.requiredBalanceRaw, access.tokenDecimals)} tokens</p><p className="mt-2 text-sm text-muted-foreground">Supply reference: block {access.supplyReferenceBlock?.toLocaleString() ?? "pending"}. Access is rechecked at most every two minutes.</p>{workspaceTab === "brief" ? <><div className="mt-5 grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
        <Select value={launches.some((launch) => launch.tokenAddress === token) ? token : undefined} onValueChange={setToken}><SelectTrigger className="w-full" aria-label="Choose indexed token"><SelectValue placeholder="Choose an indexed token" /></SelectTrigger><SelectContent>{launches.map((launch) => <SelectItem key={launch.tokenAddress} value={launch.tokenAddress}><TokenLabel token={launch} showName={false} /> · {launch.pairSymbol}</SelectItem>)}</SelectContent></Select>
    ...
    {authenticated && access?.active ? <Tabs value={workspaceTab} onValueChange={setWorkspaceTab}><TabsList aria-label="Premium research workspace" className="w-full justify-start"><TabsTrigger value="ask">Ask</TabsTrigger><TabsTrigger value="brief">Evidence</TabsTrigger><TabsTrigger value="saved">Saved research</TabsTrigger></TabsList><TabsContent value="ask" className="mt-3"><HolderResearcher key={identityVersion} accessExpiresAt={access.expiresAt} initialToken={initialToken} view="ask" /></TabsContent><TabsContent value="brief" className="mt-3 space-y-4">{payload?.dossier ? <><div className="flex justify-end"><Button variant="outline" onClick={() => setWorkspaceTab("saved")}><BookOpen />Save this window as a case</Button></div><ResearchContent data={payload.dossier} loading={loading} olderPage={olderPage} loadOlder={() => void load(payload.dossier?.history.nextCursor ?? undefined)} loadNewest={() => void load()} /></> : <div className={box}><p className="text-base leading-7 text-muted-foreground">Open an indexed token to inspect its evidence, source relationships, and research paths.</p></div>}</TabsContent><TabsContent value="saved" forceMount className="mt-3 space-y-6 data-[state=inactive]:hidden">{workspaceTab === "saved" ? <HolderResearcher key={`${identityVersion}:saved`} accessExpiresAt={access.expiresAt} initialToken={null} view="saved" /> : null}<PremiumCaseFiles key={identityVersion} dossier={payload?.dossier ?? null} accessExpiresAt={access.expiresAt} /></TabsContent></Tabs> : null}
