...
export async function loadFactoryFeed(): Promise<PonsFactoryFeed | null> {
  const row = await getD1().prepare(`SELECT f.payload_json,
    (SELECT json_group_array(json_object('token_address', token_address, 'token_symbol', token_symbol, 'token_name', token_name, 'pair_symbol', pair_symbol))
  ...
  const identities = new Map<string, { token_symbol: string | null; token_name: string | null; pair_symbol: string }>();
  const rows = JSON.parse(row.identities_json) as Array<{ token_address: string; token_symbol: string | null; token_name: string | null; pair_symbol: string }>;
  ...
  return { ...feed, events: feed.events.map((event) => ({ ...event,
    ...
    tokenName: identities.get(event.tokenAddress)?.token_name || event.tokenName,
