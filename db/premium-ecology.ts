...
export async function readPremiumEcology(input: { token: string; quoteAddress: string; quoteSymbol: string; through: number; collectedAt: string | null; holder: PonsTokenEvidence | null; current: boolean }): Promise<ResearchEcology> {
  ...
  const peers: EcologyPeer[] = [
    ...v2.results.map(row => ({ address: row.token_address, symbol: row.token_symbol || row.token_address.slice(0, 10), source: "PONS V2", relationship: "Exact quote-contract match", observedAt: iso(row.block_timestamp * 1000)!, url: robinhoodExplorer.token(row.token_address) })),
    ...v1.results.map(row => ({ address: row.token_address, symbol: row.token_symbol || row.token_address.slice(0, 10), source: `PONS ${row.generation}`, relationship: "Exact quote-contract match", observedAt: iso(row.block_timestamp * 1000)!, url: robinhoodExplorer.token(row.token_address) })),
    ...pair.results.map(row => ({ address: row.contract_address, symbol: row.symbol, source: row.protocol, relationship: "Same named habitat · taxonomy match only", observedAt: iso(row.observed_at)!, url: robinhoodExplorer.token(row.contract_address) })),
  ];
  const sources: EcologySource[] = [
    { id: "pons", label: "PONS event index", status: "recorded", observedAt: input.collectedAt, relationship: "Launch and quote contract", detail: `PONS peers use the same quote contract and are limited to committed launches through block ${input.through.toLocaleString()}.`, url: robinhoodExplorer.token(input.token), stale: true },
    { id: "holder", label: input.holder?.sampleBasis === "indexed-actors" ? "Indexed-actor balance sample" : "Explorer holder sample", status: input.holder?.observedAt ? "recorded" : "missing", observedAt: input.holder?.observedAt ?? null, relationship: "Exact token contract", detail: input.holder?.observedAt ? `${input.holder.holderSampleSize} wallets sampled. ${input.holder.holdersComplete ? "Returned distribution complete." : "Partial distribution."}` : "No dated holder sample is available.", url: `${robinhoodExplorer.token(input.token)}#balances`, stale: true },
